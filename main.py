...
async def on_message(message):
    ...
    # تحويل الأوامر المكتوبة بمسافة إلى الصياغة الصحيحة داخلياً
...
async def نك(ctx, member: discord.Member = None, *, new_name: str = None):
    ...
    if not has_admin_privilege:
        await ctx.send("❌ عذراً، هذا الأمر مخصص للإدارة فقط!", delete_after=5)
        return
...
async def remove_nick(ctx, member: discord.Member = None):
    ...
    if member is None:
        await ctx.send("❌ الرجاء تحديد العضوة. مثال: `!حذف نك @العضوة`", delete_after=5)
        return
    try:
...
async def streak_mention_command(ctx, member: discord.Member = None, number: str = None):
    ...
    if member is None or number is None:
        await ctx.send("❌ مثال: `!ستريك @العضوة 5`", delete_after=5)
        return
...
class ChairsGameView(discord.ui.View):
    ...
    def make_callback(self, index):
        async def button_callback(interaction: discord.Interaction):
            if interaction.user not in self.players: 
                await interaction.response.send_message("❌ أنتِ لستِ من ضمن اللاعبات المشاركات في هذه الجولة!", ephemeral=True)
                return
            if interaction.user in self.claimed_players: 
                await interaction.response.send_message("❌ لقد حجزتِ كرسياً بالفعل!", ephemeral=True)
                return
    ...
    async def end_round(self):
        ...
        if loser:
            ...
            await self.ctx.send(f"💀 **انتهت الجولة!** {loser.mention} لم تجد كرسياً وتم إقصاؤها! 🚷")
            
        if len(self.players) == 1:
            await self.ctx.send(f"👑✨ **الفائزة النهائية والملكة على الكراسي هي: {self.players[0].mention}!** ✨👑")
        elif len(self.players) > 1:
            await asyncio.sleep(3)
            next_view = ChairsGameView(self.ctx, self.players)
            await self.ctx.send("🎵 **الموسيقى تعمل مجدداً... استعدوا!**")
            await asyncio.sleep(random.randint(3, 7))
            await self.ctx.send("🪑 **توقفت الموسيقى!! اضغطوا الكراسي فورا!**", view=next_view)

    async def on_timeout(self):
        if self.game_over: return
        self.game_over = True
        
        # إقصاء كل من لم يضغط على الكرسي في الوقت المحدد
        losers = [p for p in self.players if p not in self.claimed_players]
        for l in losers:
            self.players.remove(l)
            
        await self.ctx.send("⏱️ **انتهى الوقت!** تم إقصاء من لم يجلس في الوقت المحدد.")
        
        if len(self.players) == 1:
            await self.ctx.send(f"👑✨ **الفائزة النهائية هي: {self.players[0].mention}!** ✨👑")
        elif len(self.players) > 1:
            next_view = ChairsGameView(self.ctx, self.players)
            await self.ctx.send("🪑 **الجولة التالية تبدأ الآن!**", view=next_view)
        else:
            await self.ctx.send("❌ انتهت اللعبة بدون فائز بسبب انتهاء الوقت دون تفاعل كافٍ.")

@bot.command()
@has_allowed_role()
async def كراسي(ctx):
    if is_paused: return
    await ctx.send("🎵 **لعبة الكراسي الموسيقية بدأت!** 🎵\nاضغطي على زر **(انضمام 🎮)** للمشاركة في اللعبة!")
    ...
    class JoinView(discord.ui.View):
        ...
        async def join_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            await interaction.response.send_message("✅ تم تسجيلكِ بنجاح في اللعبة!", ephemeral=True)
            
    join_view = JoinView()
    msg = await ctx.send("انتظار انضمام اللاعبات (المجال مفتوح لمدة 20 ثانية):", view=join_view)
    
    await asyncio.sleep(20.0)
    
    # إزالة الأزرار بعد انتهاء وقت التسجيل لتفادي الضغط المتأخر
    for item in join_view.children:
        item.disabled = True
    await msg.edit(view=join_view)
    
    if len(registered_players) < 2:
        await ctx.send("❌ تم إلغاء اللعبة، عدد المشتركات غير كافٍ (يجب تسجيل لاعبين على الأقل).")
        return
        
    await ctx.send(f"تم تسجيل `{len(registered_players)}` لاعبات. بدأت الموسيقى الآن... 🎵")
    await asyncio.sleep(random.randint(4, 8)) # انتظار عشوائي قبل توقف الموسيقى
    
    first_round_view = ChairsGameView(ctx, registered_players)
    await ctx.send("🪑 **توقفت الموسيقى فجأة!! أسرعوا واحجزوا الكراسي!**", view=first_round_view)
...
if TOKEN:
    ...
    print("خطأ: لم يتم العثور على متغير TOKEN في إعدادات البيئة المحيطة (Environment Variables)!")
